...
| 阶段 (Stage)                                  | 安全问题 (Security Issue)                      | 简要原理与利用方式                                                                                                                | 核心缓解措施 (Key Mitigations)                                               |
| ------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| ​**​AS-REQ & AS-REP​**​  <br>_(认证服务交换)_     | ​**​AS-REP Roasting​**​                    | 利用域用户账号设置了“​**​不需要Kerberos预身份验证​**​” (`DONT_REQUIRE_PREAUTH`) 的属性，攻击者可直接为其请求TGT，离线破解加密的Session Key部分获取密码哈希。              | ​**​强制所有域用户启用预认证​**​，定期审计账户配置。                                         |
|                                             | ​**​密码喷洒攻击 (Password Spraying)​**​         | 使用少数几个常用密码（如`Company2023!`, `Welcome1`）对大量用户名进行尝试，利用AS-REP返回的错误码差异规避账户锁定策略。                                              | 实施​**​强密码策略​**​，配置合理的​**​账户锁定阈值​**​，启用​**​多因素认证(MFA)​**​。              |
|                                             | ​**​用户名枚举 (User Enumeration)​**​           | 根据KDC对存在与不存在的用户名返回的不同错误码（如 `KDC_ERR_PREAUTH_REQUIRED`与 `KDC_ERR_C_PRINCIPAL_UNKNOWN`）来枚举有效域用户名。                          | 监控异常的AS-REQ请求频率。                                                       |
|                                             | ​**​哈希传递 (PtH)​**​  <br>​**​密钥传递 (PtK)​**​ | 使用获取的用户​**​NTLM Hash​**​或​**​AES Key​**​（而非明文密码）加密时间戳完成预认证，直接向KDC申请TGT。                                                  | 实施​**​强密码策略​**​，启用​**​受保护用户组​**​（Protected Users），部署​**​LAPS​**​。      |
| ​**​TGS-REQ & TGS-REP​**​  <br>_(票据授予服务交换)_ | ​**​Kerberoasting​**​                      | 任何域用户均可为注册了​**​SPN​**​的服务账户请求服务票据（ST）。若服务账户密码强度弱，可离线暴力破解ST的加密部分（常为RC4加密）以获取服务账户密码哈希。                                     | 为服务账户设置​**​强密码​**​（≥25位随机），优先使用​**​AES加密​**​，限制账户的​**​委派权限​**​，定期轮换密码。 |
|                                             | ​**​票据传递 (Pass-the-Ticket, PtT)​**​        | 从内存中窃取或从磁盘转储（`lsass.dmp`）合法的TGT或ST票据，将其注入到当前会话中，冒充该用户身份访问服务资源。                                                           | 限制票据​**​有效期​**​，启用票据​**​时间戳检查​**​，部署EDR监控异常票据使用。                       |
|                                             | ​**​S4U委派攻击 (约束委派)​**​                     | 控制一个配置了约束委派的服务账户后，利用其TGT，通过​**​S4U2Self​**​获取一个可转发的票据，再通过​**​S4U2Proxy​**​以​**​任意用户​**​身份（如域管理员）申请访问指定服务（如域控的CIFS服务）的ST。 | ​**​最小化委派权限​**​，定期审计`msDS-AllowedToDelegateTo`属性，监控事件ID 4769。          |
|                                             | ​**​基于资源的约束委派 (RBCD)​**​                   | 攻击者若拥有修改某计算机账户`msDS-AllowedToActOnBehalfOfOtherIdentity`属性的权限，可将自身控制的服务账户添加到委派列表，然后通过S4U协议模拟高权限用户访问该服务。                  | 严格管理计算机账户的写权限，优先使用RBCD而非传统约束委派。                                        |
| ​**​AP-REQ & AP-REP​**​  <br>_(应用服务交换)_     | ​**​白银票据 (Silver Ticket)​**​               | 获取​**​服务账户​**​的密码哈希后，直接伪造针对该特定服务的ST（无需与KDC交互）。服务器使用自身密钥解密ST验证通过（若未开启严格的PAC验证）。                                           | 定期​**​轮换服务账户密码​**​，对关键服务​**​启用并强制PAC验证​**​。                            |
|                                             | ​**​PAC 伪造 (MS14-068)​**​                  | KDC未正确验证PAC中的特权属性证书签名，允许攻击者篡改PAC，将自己添加到高权限组（如域管理员），从而提升权限。                                                               | ​**​安装KB3011780补丁​**​，确保所有域控及时更新。                                      |
| ​**​协议通用及跨阶段问题​**​                          | ​**​黄金票据 (Golden Ticket)​**​               | 获取域控`krbtgt`账户的NTLM哈希后，可伪造任意用户的TGT，从而生成访问域内任何服务的ST，获得持久性的域内完全控制权。                                                        | 定期​**​轮换krbtgt账户密码​**​（需按微软官方方案操作），严格限制对域控的访问。                         |
|                                             | ​**​重放攻击 (Replay Attack)​**​               | 在票据有效时间窗口内（通常5分钟），捕获并重新发送有效的认证报文。                                                                                        | 确保域内所有主机与域控​**​时间同步​**​（NTP），限制票据​**​合理有效期​**​。                        |
